      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex flex-1 justify-end gap-4 lg:gap-6 items-center min-w-0">
            <Link to="/" className={linkClass('/')}>{lang === 'hi' ? 'होम' : 'Home'}</Link>
            <Link to="/services" className={linkClass('/services')}>{lang === 'hi' ? 'सर्विसेज' : 'Services'}</Link>
            <Link to="/how-it-works" className={linkClass('/how-it-works')}>{lang === 'hi' ? 'कैसे काम करता है' : 'How it Works'}</Link>
            <Link to="/worker-onboarding" className={linkClass('/worker-onboarding')}>{lang === 'hi' ? 'वर्कर बनें' : 'Become Worker'}</Link>
      ...
      {isOpen && (
        <div className="md:hidden bg-card-white border-t">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <Link to="/" onClick={() => setIsOpen(false)} className="block px-3 py-3 text-text-gray font-medium hover:bg-bg-warm rounded-md">Home</Link>
